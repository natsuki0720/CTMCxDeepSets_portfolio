...
class DeepSets_varSets_forDiagnel(nn.Module):
    def __init__(self, num_categories=4, embedding_dim=2):
        ...
        # 各サンプルの特徴抽出層（適度にユニット数増加）
        self.fc1 = nn.Linear(embedding_dim * 2 + 1, 48)  # ★ 32 → 48 に増加
        self.bn1 = nn.BatchNorm1d(48)
        self.dropout1 = nn.Dropout(p=0.1)

        self.fc2 = nn.Linear(48, 24)  # ★ 16 → 24 に増加
        self.bn2 = nn.BatchNorm1d(24)
        self.dropout2 = nn.Dropout(p=0.3)

        # 集約後の層（控えめに増加）
        self.fc3 = nn.Linear(24, 96)  # ★ 64 → 96 に増加
        self.dropout3 = nn.Dropout(p=0.3)

        self.fc4 = nn.Linear(96, 48)  # ★ 32 → 48 に増加
        self.dropout4 = nn.Dropout(p=0.1)

        self.fc5 = nn.Linear(48, 3)  # ★ 出力層は 3 のまま
    ...
    def forward(self, state, delta_t, lengths):
        ...
        output = F.softplus(self.fc5(x))  # 出力を正値に制約
